...
│     ├─ live/                  # /live read model: stats.ts (cached ESPN index) → assemble.ts + staleness.ts (pure) → query.ts (reads only)
...
│     ├─ schedule/              # syncSeasonSchedule → upserts nfl_games; preseason.ts = week-namespace helpers (isolation only)
