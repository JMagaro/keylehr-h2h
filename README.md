...
│     ├─ live/                  # /live read model: stats.ts (cached ESPN index) → assemble.ts + staleness.ts + minutes.ts + projection.ts (all pure) → query.ts (reads only)
